    ...
    async function askQuestion(question) {
        let response = await fetch("https://api.w-davis.com/nightingale/answer", {
            ...
            headers: {
                ...
                "x-wd-api-key": "pk-EcI1Nn2daolEuHWe5G0pE7shYEQTaoNr9kwWuESXTPC7b1wBzvfd0QH0alKGkSAV", //DO NOT COMMIT
            },
            mode: "cors",
            redirect: "follow",
...
function NightingaleBubble() {
    ...
    return (
        <div className="bottom-8 right-8 hidden lg:block fixed">
            {chatOpen && (
                <AnimatePresence>
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="absolute bottom-20 right-0 z-10 flex h-fit w-[85vw] flex-col items-center lg:right-24 lg:w-[30rem]"
                    >
                        <NightingaleChat />
                    </motion.div>
                </AnimatePresence>
